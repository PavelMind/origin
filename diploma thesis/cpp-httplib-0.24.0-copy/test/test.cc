...
protected:
  ...
  const std::string pathname_{"./httplib_S-server.sock"};
  const std::string pattern_{"/hi"};
  const std::string content_{"Hello World!"};
};

TEST_F(UnixSocketTest, pathname) {
  httplib_S::Server svr;
  svr.Get(pattern_, [&](const httplib_S::Request &, httplib_S::Response &res) {
...
TEST_F(UnixSocketTest, PeerPid) {
  ...
  svr.Get(pattern_, [&](const httplib_S::Request &req, httplib_S::Response &res) {
    res.set_content(content_, "text/plain");
...
TEST_F(UnixSocketTest, abstract) {
  constexpr char svr_path[]{"\x00httplib_S-server.sock"};
...
TEST_F(UnixSocketTest, HostHeaderAutoSet) {
  ...
  // connections
  httplib_S::Client cli{pathname_};
  cli.set_address_family(AF_UNIX);
  ASSERT_TRUE(cli.is_valid());
...
TEST(EncodeQueryParamTest, ParseUnescapedChararactersTest) {
  ...
  EXPECT_EQ(httplib_S::encode_uri_component(unescapedCharacters), "-_.!~*'()");
}

TEST(EncodeQueryParamTest, ParseReservedCharactersTest) {
  string reservedCharacters = ";,/?:@&=+$";

  EXPECT_EQ(httplib_S::encode_uri_component(reservedCharacters),
...
TEST(EncodeQueryParamTest, TestUTF8Characters) {
  ...
  EXPECT_EQ(httplib_S::encode_uri_component(chineseCharacters),
            "%E4%B8%AD%E5%9B%BD%E8%AA%9E");

  EXPECT_EQ(httplib_S::encode_uri_component(russianCharacters),
            "%D0%B4%D0%BE%D0%BC");

  EXPECT_EQ(httplib_S::encode_uri_component(brazilianCharacters), "%C3%B3culos");
}
...
TEST(EncodeUriComponentTest, TestPathComponentEncoding) {
  ...
  EXPECT_EQ(httplib_S::encode_uri_component(pathWithAmpersand),
...
TEST(EncodeUriTest, ParseUnescapedChararactersTest) {
  ...
  EXPECT_EQ(httplib_S::encode_uri(unescapedCharacters), "-_.!~*'()");
}

TEST(EncodeUriTest, ParseReservedCharactersTest) {
  string reservedCharacters = ";,/?:@&=+$#";

  EXPECT_EQ(httplib_S::encode_uri(reservedCharacters), ";,/?:@&=+$#");
}
...
TEST(EncodeUriTest, TestUTF8Characters) {
  ...
  EXPECT_EQ(httplib_S::encode_uri(chineseCharacters),
            "%E4%B8%AD%E5%9B%BD%E8%AA%9E");

  EXPECT_EQ(httplib_S::encode_uri(russianCharacters), "%D0%B4%D0%BE%D0%BC");

  EXPECT_EQ(httplib_S::encode_uri(brazilianCharacters), "%C3%B3culos");
}
...
TEST(EncodeUriTest, TestUriWithSpacesAndSpecialChars) {
  ...
  EXPECT_EQ(httplib_S::encode_uri(uri),
...
TEST(DecodeUriComponentTest, ParseEncodedChararactersTest) {
  ...
  EXPECT_EQ(httplib_S::decode_uri_component(encodedString), ";,/?:@&=+$");
}

TEST(DecodeUriComponentTest, ParseUnescapedChararactersTest) {
  string unescapedCharacters = "-_.!~*'()";

  EXPECT_EQ(httplib_S::decode_uri_component(unescapedCharacters), "-_.!~*'()");
}
...
TEST(DecodeUriComponentTest, TestUTF8Characters) {
  ...
  EXPECT_EQ(httplib_S::decode_uri_component(encodedChinese), u8"中国語");
  EXPECT_EQ(httplib_S::decode_uri_component(encodedRussian), u8"дом");
  EXPECT_EQ(httplib_S::decode_uri_component(encodedBrazilian), u8"óculos");
}

TEST(DecodeUriComponentTest, TestPathComponentDecoding) {
  string encodedPath = "Piri%20Tommy%20Villiers%20-%20on%20%26%20on";

  EXPECT_EQ(httplib_S::decode_uri_component(encodedPath),
...
TEST(DecodeUriTest, ParseEncodedChararactersTest) {
  ...
  EXPECT_EQ(httplib_S::decode_uri(encodedString), " \"<>\\^`{}|");
}

TEST(DecodeUriTest, ParseUnescapedChararactersTest) {
  string unescapedCharacters = "-_.!~*'();,/?:@&=+$#";

  EXPECT_EQ(httplib_S::decode_uri(unescapedCharacters), "-_.!~*'();,/?:@&=+$#");
}
...
TEST(DecodeUriTest, TestUTF8Characters) {
  ...
  EXPECT_EQ(httplib_S::decode_uri(encodedChinese), u8"中国語");
  EXPECT_EQ(httplib_S::decode_uri(encodedRussian), u8"дом");
  EXPECT_EQ(httplib_S::decode_uri(encodedBrazilian), u8"óculos");
}
...
TEST(DecodeUriTest, TestCompleteUri) {
  ...
  EXPECT_EQ(
      httplib_S::decode_uri(encodedUri),
      "https://example.com/path with spaces/file name.html?q=hello world");
}
...
TEST(DecodeUriTest, TestRoundTripWithEncodeUri) {
  string original =
      "https://example.com/path with spaces/file name.html?q=hello world";
  string encoded = httplib_S::encode_uri(original);
  string decoded = httplib_S::decode_uri(encoded);
...
TEST(DecodeUriComponentTest, TestRoundTripWithEncodeUriComponent) {
  ...
  string encoded = httplib_S::encode_uri_component(original);
  string decoded = httplib_S::decode_uri_component(encoded);
...
TEST(ParseMultipartBoundaryTest, ValueWithQuotesAndCharset) {
  string content_type =
      "multipart/mixed; boundary=\"cpp-httplib_S-multipart-data\"; charset=UTF-8";
  string boundary;
  auto ret = detail::parse_multipart_boundary(content_type, boundary);
  EXPECT_TRUE(ret);
  EXPECT_EQ(boundary, "cpp-httplib_S-multipart-data");
}
...
protected:
  ...
  virtual void SetUp() {
    ...
    svr_.Get(
        "/chunked", [this](const httplib_S::Request &, httplib_S::Response &res) {
          res.set_chunked_content_provider(
              "image/jpeg", [this](size_t offset, httplib_S::DataSink &sink) {
...
TEST(RoutingHandlerTest, PreAndPostRoutingHandlers) {
  ...
  svr.set_pre_routing_handler([](const Request &req, Response &res) {
    if (req.path == "/routing_handler") {
      ...
      return httplib_S::Server::HandlerResponse::Handled;
    }
    return httplib_S::Server::HandlerResponse::Unhandled;
  });
...
protected:
  ...
  virtual void SetUp() {
    ...
    svr_.Get("/hi",
        ...
        .Delete("/issue609",
                [](const httplib_S::Request &, httplib_S::Response &res,
...
TEST_F(ServerTest, StaticFileRanges) {
  ...
  EXPECT_TRUE(
      res->get_header_value("Content-Type")
          .find(
              "multipart/byteranges; boundary=--cpp-httplib_S-multipart-data-") ==
      0);
...
TEST_F(ServerTest, LongHeader) {
  ...
  req.headers.emplace("User-Agent", "cpp-httplib_S/0.1");
...
TEST_F(ServerTest, HeaderCountSecurityTest) {
  ...
  httplib_S::Headers attack_headers;
...
TEST(GzipDecompressor, ChunkedDecompression) {
  ...
  {
    httplib_S::detail::gzip_compressor compressor;
  ...
  {
    httplib_S::detail::gzip_decompressor decompressor;
...
TEST(ZstdDecompressor, ChunkedDecompression) {
  ...
  std::string compressed_data;
  {
    httplib_S::detail::zstd_compressor compressor;
    bool result = compressor.compress(
        data.data(), data.size(),
        /*last=*/true,
        [&](const char *compressed_data_chunk, size_t compressed_data_size) {
  ...
  std::string decompressed_data;
  {
    httplib_S::detail::zstd_decompressor decompressor;
...
TEST(ServerStopTest, ClientAccessAfterServerDown) {
  httplib_S::Server svr;
  svr.Post("/hi",
           [&](const httplib_S::Request & /*req*/, httplib_S::Response &res) {
             res.status = StatusCode::OK_200;
           });
...
TEST(KeepAliveTest, MaxCount) {
  ...
  svr.Get("/hi", [](const httplib_S::Request &, httplib_S::Response &res) {
    res.set_content("Hello World!", "text/plain");
  });
...
TEST(KeepAliveTest, SSLClientReconnectionPost) {
  ...
  svr.Post("/hi", [](const httplib_S::Request &, httplib_S::Response &res) {
    res.set_content("Hello World!", "text/plain");
  });
...
TEST(HostAndPortPropertiesTest, NoSSL) {
  httplib_S::Client cli("www.google.com", 1234);
...
TEST(SSLClientTest, SetInterfaceWithINET6) {
  auto cli = std::make_shared<httplib_S::Client>("https://httpbin.org");
...
TEST(ClientImplMethods, GetSocketTest) {
  httplib_S::Server svr;
  svr.Get("/", [&](const httplib_S::Request & /*req*/, httplib_S::Response &res) {
    res.status = StatusCode::OK_200;
  });
  ...
  {
    httplib_S::Client cli("http://127.0.0.1:3333");
    cli.set_keep_alive(true);

    // Use the behavior of cpp-httplib_S of opening the connection
...
TEST(MultipartFormDataTest, DataProviderItems) {
...

  {
    ...
    providers.push_back({"name3",
                         [&](size_t offset, httplib_S::DataSink &sink) -> bool {
...
TEST(MultipartFormDataTest, LargeHeader) {
  ...
  auto boundary = std::string("cpp-httplib_S-multipart-data");
