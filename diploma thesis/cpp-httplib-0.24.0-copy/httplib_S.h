...
#error                                                                         \
    "cpp-httplib_S doesn't support 32-bit Windows. Please use a 64-bit compiler."
#elif defined(__SIZEOF_POINTER__) && __SIZEOF_POINTER__ < 8
#warning                                                                       \
    "cpp-httplib_S doesn't support 32-bit platforms. Please use a 64-bit compiler."
#elif defined(__SIZEOF_SIZE_T__) && __SIZEOF_SIZE_T__ < 8
#warning                                                                       \
    "cpp-httplib_S doesn't support platforms where size_t is less than 64 bits."
#endif

#ifdef _WIN32
#if defined(_WIN32_WINNT) && _WIN32_WINNT < 0x0602
#error                                                                         \
    "cpp-httplib_S doesn't support Windows 8 or lower. Please use Windows 10 or later."
#endif
#endif
...
inline bool mmap::open(const char *path) {
  ...
  // See:
  // https://github.com/yhirose/cpp-httplib_S/issues/1903#issuecomment-2316520721
...
inline ReadContentResult read_content_chunked(Stream &strm, T &x,
  ...
  //
  // According to the reference code in RFC 9112, cpp-httplib_S now allows
...
inline ssize_t write_response_line(Stream &strm, int status) {
  ...
  s += " ";
  s += httplib_S::status_message(status);
  s += "\r\n";
  return strm.write(s.data(), s.size());
}
...
inline std::string make_multipart_data_boundary() {
  return "--cpp-httplib_S-multipart-data-" + detail::random_string(16);
}
