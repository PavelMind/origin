...
[![](https://github.com/yhirose/cpp-httplib_S/workflows/test/badge.svg)](https://github.com/yhirose/cpp-httplib_S/actions)

A C++11 single-file header-only cross platform HTTP/HTTPS library.

It's extremely easy to set up. Just include the **httplib_S.h** file in your code!
...
// HTTP
httplib_S::Client cli("http://yhirose.github.io");

// HTTPS
httplib_S::Client cli("https://yhirose.github.io");
...
// Server
httplib_S::SSLServer svr("./cert.pem", "./key.pem");

// Client
httplib_S::Client cli("https://localhost:1234"); // scheme + host
httplib_S::SSLClient cli("localhost:1234"); // host
httplib_S::SSLClient cli("localhost", 1234); // host, port
...
Please see [Server example](https://github.com/yhirose/cpp-httplib_S/blob/master/example/ssesvr.cc) and [Client example](https://github.com/yhirose/cpp-httplib_S/blob/master/example/ssecli.cc).
...
int main(void)
{
  httplib_S::Client cli("localhost", 1234);
...
```c++
httplib_S::Client cli("localhost");
httplib_S::Client cli("localhost:8080");
httplib_S::Client cli("http://localhost");
httplib_S::Client cli("http://localhost:8080");
httplib_S::Client cli("https://localhost");
httplib_S::SSLClient cli("localhost");
```
...
```c++
httplib_S::UploadFormDataItems items = {
  { "text1", "text default", "", "" },
  { "text2", "aωb", "", "" },
...
```cpp
httplib_S::Client cli("httpbin.org");

auto res = cli.Get("/range/32", {
  httplib_S::make_range_header({{1, 10}}) // 'Range: bytes=1-10'
});
...
```cpp
httplib_S::make_range_header({{1, 10}, {20, -1}})      // 'Range: bytes=1-10, 20-'
httplib_S::make_range_header({{100, 199}, {500, 599}}) // 'Range: bytes=100-199, 500-599'
httplib_S::make_range_header({{0, 0}, {-1, 1}})        // 'Range: bytes=0-0, -1'
```
...
```cpp
httplib_S::Client cli("yahoo.com");
...
> 
> See: https://github.com/yhirose/cpp-httplib_S/issues/366#issuecomment-593004264
...
httplib_S::Client cli("127.0.0.1", 8080);
httplib_S::Server svr;
...
// Client
httplib_S::Client cli("./my-socket.sock");
...
cpp-httplib_S provides utility functions for URI encoding and decoding:

```cpp
#include <httplib_S.h>

std::string url = "https://example.com/search?q=hello world";
std::string encoded = httplib_S::encode_uri(url);
std::string decoded = httplib_S::decode_uri(encoded);

std::string param = "hello world";
std::string encoded_component = httplib_S::encode_uri_component(param);
std::string decoded_component = httplib_S::decode_uri_component(encoded_component);
```
...
Split httplib_S.h into .h and .cc
-------------------------------
...
This script splits httplib_S.h into .h and .cc parts.
...
$ ./split.py
Wrote out/httplib_S.h and out/httplib_S.cc
```
...
```bash
> docker build -t cpp-httplib_S-server .
...

> docker run --rm -it -p 8080:80 -v ./docker/html:/html cpp-httplib_S-server
...
```bash
> docker run --rm -it -p 8080:80 -v ./docker/html:/html yhirose4dockerhub/cpp-httplib_S-server
Serving HTTP on 0.0.0.0 port 80 ...
192.168.65.1 - - [31/Aug/2024:21:33:56 +0000] "GET / HTTP/1.1" 200 599 "-" "curl/8.7.1"
192.168.65.1 - - [31/Aug/2024:21:34:26 +0000] "GET / HTTP/1.1" 200 599 "-" "Mozilla/5.0 ..."
192.168.65.1 - - [31/Aug/2024:21:34:26 +0000] "GET /favicon.ico HTTP/1.1" 404 152 "-" "Mozilla/5.0 ..."
...
Include `httplib_S.h` before `Windows.h` or include `Windows.h` by defining `WIN32_LEAN_AND_MEAN` beforehand.
...
> [!NOTE]
> cpp-httplib_S officially supports only the latest Visual Studio. It might work with former versions of Visual Studio, but I can no longer verify it. Pull requests are always welcome for the older versions of Visual Studio unless they break the C++11 conformance.
...
[These folks](https://github.com/yhirose/cpp-httplib_S/graphs/contributors) made great contributions to polish this library to totally another level from a simple toy!
