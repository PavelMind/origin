...
const std::vector<std::string> addrSite::typeFile = { 
    ".jpg", ".jpeg", ".png", ".gif", ".svg", ".webp", ".mp3", ".mp4",".zip", ".xml", ".webm"};
...
void addrSite::relativeLink(std::string& newPh, const addrSite& curr) {
    ...
    int lastSl;
    if (CuPath.back() == '/' && CuPath.size() > 2)
        lastSl = CuPath.rfind('/', CuPath.size() - 2);
    else
        lastSl = CuPath.rfind('/');

    int lastPnt = CuPath.rfind('.');
    if(lastPnt != -1 &&  lastSl < lastPnt) {
        CuPath.erase(lastSl + 1);
    }
    if (CuPath.back() != '/')
        CuPath.push_back('/');
...
bool addrSite::ref() const {
    ...
    if (p != -1) {
        ...
        if (std::find(typeFile.begin(), typeFile.end(), strFile) != typeFile.end())
            notFile = false;
    }    
    return prot && notFile;
}
