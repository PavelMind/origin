...
void indexator::cleanerText(std::string& text, std::string& title, const addrSite& currS, std::vector<addrSite>& links){
        ...
        if (text[i] == '<') {
            ...
            if (text.substr(i + 1, 2) == "a ") {
                ...
                if (strAddr.size() > 0) {                    
                    ...
                    bool noInList = std::find(linkLoc.begin(), linkLoc.end(), st) == linkLoc.end();
...
void indexator::indexation(std::string& text, const addrSite& currSite, std::vector<addrSite>& link) {
    links.clear();
    std::string title{};
    cleanerText(text, title, currSite, link);
